...
import com.deep.coral.annotations.ScheduleHoliday;
import com.deep.coral.core.task.BasicTask;
import com.deep.coral.core.task.HolidayTask;
...
/**
 * <h2>解析 ScheduleHoliday 注解</h2>
 *
 * @author Create by liuwenhao on 2022/2/10 11:25
 */
@Component
@ConditionalOnBean(ScheduledThreadPoolExecutor.class)
public class ExplainHoliday implements Explain {

    private static final Logger logger = LoggerFactory.getLogger(ExplainHoliday.class);
    @Autowired
    ...
    public List<TaskGuardian> explain(String[] packageName) {
        ...
        Set<Method> methods = reflections.getMethodsAnnotatedWith(ScheduleHoliday.class);
    ...
    private TaskGuardian getTask(Method method) {
        // 获取注解
        ScheduleHoliday schedule = method.getAnnotation(ScheduleHoliday.class);
        HolidayEnum holiday = schedule.holiday();
        boolean scheduleOnce = schedule.isOnce();
        String name = schedule.name();
        if (CharSequenceUtil.isBlank(name)) {
            name = method.getClass().getSimpleName() + "#" + method.getName();
        }
        // 任务周期或循环配置
        BasicTask taskConfig = new HolidayTask(holiday, scheduleOnce);
        // 任务体
        try {
